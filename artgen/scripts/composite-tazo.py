...
Reads artgen/output/ → creates tazo discs → saves to public/tazos-generated/
→ registers in SQLite database.
"""

import json, os, sqlite3, math, io
...
def make_disc_bg(franchise_slug, rarity_key):
    """Draw tazo disc background using Pillow."""
    ...
    # Outer disc fill (radial gradient approximation)
    ...
    # Creature area background
    inner_r = RADIUS - 60
    # Semi-transparent inner bg
    overlay = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    overlay_draw = ImageDraw.Draw(overlay)
    overlay_draw.ellipse([CENTER - inner_r, CENTER - inner_r, CENTER + inner_r, CENTER + inner_r],
                         fill=(f["bgLight"][0], f["bgLight"][1], f["bgLight"][2], 80))
...
def remove_white_bg(img):
    """Convert near-white/light pixels to transparent."""
    img = img.convert("RGBA")
    data = img.load()
    
    # Sample corners to detect background color
    samples = [data[0, 0], data[0, SIZE-1], data[SIZE-1, 0], data[SIZE-1, SIZE-1],
               data[SIZE//2, 5], data[5, SIZE//2]]
    avg_r = sum(s[0] for s in samples) / len(samples)
    avg_g = sum(s[1] for s in samples) / len(samples)
    avg_b = sum(s[2] for s in samples) / len(samples)
    
    print(f"    Background sample: RGB({int(avg_r)},{int(avg_g)},{int(avg_b)})")
    
    for y in range(SIZE):
        for x in range(SIZE):
            r, g, b, a = data[x, y]
            # If pixel is close to the sampled background color
            dist = abs(r - avg_r) + abs(g - avg_g) + abs(b - avg_b)
            if dist < 80:  # Within threshold
                # Feather the alpha based on distance
                alpha = max(0, min(255, int(dist / 80 * 255)))
...
def composite_tazo(creature_img, franchise_slug, rarity_key):
    ...
    # Resize creature to fit within disc (~65% of diameter)
    creature_size = int(RADIUS * 1.4)
...
def creature_exists_in_db(db, creature_id):
    """Check if a tazo with this name/slug already exists."""
    name, _ = get_tazo_name(creature_id)
...
def process_creature(db, creature_id, counters, variant="v01"):
    ...
    art_path = OUTPUT_DIR / line / creature_dir_name / f"{creature_id}-{variant}.png"
    
    if not art_path.exists():
        print(f"  ⚠️  Art not found: {art_path}")
        return False
    
    print(f"  🎨 {creature_id} ({name}) — {line} {rarity}")
    
    # Load creature image
    creature_img = Image.open(art_path)
    
    # Adjust: if it's 1024x1024, use as-is
    ...
    if franchise_slug not in counters:
        cur = db.execute(f"SELECT COUNT(*) FROM Tazo WHERE franchiseId = (SELECT id FROM Franchise WHERE slug = ?)", (franchise_slug,))
    ...
    # DB operations
    franchise_id = get_or_create_franchise(db, franchise_slug, f_config["collectionName"].split(" ")[0])
    ...
    # Check if tazo already exists
    if creature_exists_in_db(db, creature_id):
        # Update imageUrl
        ...
        print(f"    ✅ Created DB record: {tazo_id}")
...
def main():
    ...
    # Find which have generated art
    pending = []
    for c in creatures:
        name = c["name"].lower().replace(" ", "-")
        art_path = OUTPUT_DIR / c["line"] / f"{c['id']}-{name}" / f"{c['id']}-v01.png"
        if art_path.exists():
            pending.append(c["id"])
    
    print(f"🔧 Composite Tazo Generator")
    print(f"   Found {len(pending)} creatures with generated art\n")
    ...
    for i, cid in enumerate(pending):
        try:
            ...
            print(f"  ❌ Failed: {e}")
    db.close()
    print(f"\n✨ Done! {success}/{len(pending)} composited + registered.")
