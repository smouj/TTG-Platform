...
// Matches the launcher-view footer: dark #1a1a1a bar with
// platform badges, page links, social icons, copyright.
// ============================================================
...
function PlatformBadge({ icon, label }: { icon: any; label: string }) {
  const Icon = icon
  return (
    <span className="flex items-center gap-1 px-2 py-1 border border-white/10 text-[#FFCC00]/40">
      <Icon className="w-3 h-3" />
      <span className="text-[9px] font-black text-[#FFCC00]/30 uppercase tracking-wider">
        {label}
      </span>
    </span>
  )
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between px-4 sm:px-6 py-2 gap-2">
        <div className="flex items-center gap-3 sm:gap-4 flex-wrap justify-center">
          ...
          {/* GitHub */}
          <a
            href="https://github.com/smouj/Trading-Tazos-Game"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="GitHub"
            className="flex items-center justify-center w-6 h-6 rounded-full border border-white/15 text-zinc-400 hover:text-white hover:border-white/40 transition-all"
          >
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3">
      ...
      <div className="text-center px-4 pb-2">
        <p className="text-[7px] font-bold text-white/10 uppercase leading-relaxed">
          Independent fictional digital tazo game. Not affiliated with any
          third-party brand.
        </p>
      </div>
    </footer>
