    ...
    return (
      <AdminShell accentColor='var(--ttg-purple)'>
        <div className="flex items-center justify-center py-20">
          <Loader2 className="w-8 h-8 animate-spin text-ttg-purple" />
        </div>
      </AdminShell>
    );
  }
    ...
    <AdminShell accentColor='var(--ttg-purple)'>
      <div className="max-w-full mx-auto px-4 py-4">
        <div className="flex items-center gap-3 mb-4">
          <LayoutGrid className="w-5 h-5 text-ttg-purple" />
          <h1 className="text-lg font-black uppercase text-ttg-black tracking-wider">Tazo Designer</h1>
          <span className="text-[10px] font-bold text-ttg-black/30 uppercase">Drag & drop visual editor</span>
        </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          {/* Tazo navigator */}
          {selectedTazo && (
            <div className="flex items-center gap-2 bg-white border-2 border-ttg-black px-3 py-1.5 shadow-[2px_2px_0px_var(--ttg-black)]">
              <button
                onClick={() => {
                  const idx = publishedTazos.findIndex((t) => t.slug === selectedTazo.slug);
                  if (idx > 0) setSelectedTazo(publishedTazos[idx - 1]);
                }}
                className="text-ttg-black/40 hover:text-ttg-black"
              >
                ◀
              </button>
              <span className="text-[10px] font-black text-ttg-black uppercase whitespace-nowrap">
                {selectedTazo.displayName || selectedTazo.name}
              </span>
              <span className="text-[8px] font-bold text-ttg-black/40">
          ...
          {/* View mode */}
          <div className="flex bg-white border-2 border-ttg-black p-0.5 shadow-[2px_2px_0px_var(--ttg-black)]">
            <button
              onClick={() => setViewMode("editor")}
              className={`px-3 py-1 text-[9px] font-black uppercase tracking-wider transition-all ${
                viewMode === "editor" ? "bg-ttg-purple text-white" : "text-ttg-black/40 hover:text-ttg-black"
              }`}
            >
              Editor
            </button>
            <button
              ...
              className={`px-3 py-1 text-[9px] font-black uppercase tracking-wider transition-all ${
                viewMode === "grid" ? "bg-ttg-purple text-white" : "text-ttg-black/40 hover:text-ttg-black"
              }`}
            >
            ...
            <div className="flex gap-2 mb-4">
              <button
                ...
                className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider border-2 transition-all ${
                  designerSide === "front"
                    ? "bg-ttg-yellow text-ttg-black border-ttg-black shadow-[2px_2px_0px_var(--ttg-black)]"
                    : "bg-white text-ttg-black/40 border-ttg-black/10 hover:border-ttg-yellow/50"
                }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider border-2 transition-all ${
                  designerSide === "back"
                    ? "bg-ttg-black text-ttg-yellow border-ttg-yellow shadow-[2px_2px_0px_var(--ttg-yellow)]"
                    : "bg-white text-ttg-black/40 border-ttg-black/10 hover:border-ttg-black/30"
                }`}
              >
              ...
              >
                <div className="flex items-center gap-2 ml-auto">
                  <button
                    ...
                    disabled={saving}
                    className="text-[9px] font-black uppercase tracking-wider px-3 py-1.5 bg-ttg-success text-white border-2 border-ttg-black shadow-[2px_2px_0px_var(--ttg-black)] hover:shadow-[1px_1px_0px_var(--ttg-black)] flex items-center gap-1 disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    disabled={saving}
                    className="text-[9px] font-black uppercase tracking-wider px-3 py-1.5 bg-ttg-blue text-white border-2 border-ttg-black shadow-[2px_2px_0px_var(--ttg-black)] hover:shadow-[1px_1px_0px_var(--ttg-black)] flex items-center gap-1 disabled:opacity-50"
                  >
              ...
              return (
                <div key={fs} className="mag-card p-4 border-3 border-ttg-black shadow-[4px_4px_0px_var(--ttg-black)]">
                  <h2 className="text-sm font-black uppercase tracking-wider mb-4 flex items-center gap-2">
                    ...
                    {fName}
                    <span className="text-[9px] font-bold text-ttg-black/30">({tazos.length} tazos)</span>
                  </h2>
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-3">
                    {tazos.map((tazo: any) => (
                      <button
                        ...
                        className="group flex flex-col items-center gap-1.5 p-2 hover:bg-ttg-blue/5 hover:scale-105 transition-all"
                      >
                        <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-ttg-black/10 group-hover:border-ttg-blue/50 shadow-md bg-ttg-black/5 relative">
                          <TazoDiscImage
                            src={tazo.imageUrl}
                            alt={tazo.name || ""}
                            size="100%"
                        ...
                        <div className="text-center">
                          <p className="text-[9px] font-black text-ttg-black leading-tight truncate max-w-[80px]">
                            {tazo.displayName || tazo.name}
                          </p>
                          <p className="text-[7px] font-bold text-ttg-black/30 uppercase">
        ...
        {/* Keyboard shortcuts */}
        <div className="mt-6 p-3 bg-ttg-black/5 border border-ttg-black/10 flex items-center gap-4 flex-wrap">
          <span className="text-[9px] font-black uppercase text-ttg-black/40">Shortcuts:</span>
          <span className="text-[9px] font-mono font-bold text-ttg-black/30">Click element → drag to move</span>
          <span className="text-[9px] font-mono font-bold text-ttg-black/30">Arrow keys → nudge 1px</span>
          <span className="text-[9px] font-mono font-bold text-ttg-black/30">Shift+Arrow → nudge 10px</span>
          <span className="text-[9px] font-mono font-bold text-ttg-black/30">+/- → scale element</span>
          <span className="text-[9px] font-mono font-bold text-ttg-black/30">Ctrl+←/→ → prev/next tazo</span>
        </div>
      </div>
    </AdminShell>
  );
